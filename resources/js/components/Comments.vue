...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            voteComment(comment_d, commentType, index, index2, voteType) {
                if (this.user) {
                    this.$http.post('comments/' + comment_d + '/vote', {
                        users_id: this.user.id,
                        vote: voteType
                    }).then(res => {
                        if (res.data) {
            ...
            spamComment(comment_id, commentType, index, index2) {
                console.log("spam here");
                if (this.user) {
                    this.$http.post('comments/' + comment_d + '/spam', {
                        users_id: this.user.id,
                    }).then(res => {
                        if (commentType == 'directcomment') {
